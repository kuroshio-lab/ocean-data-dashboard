...
class TemperatureObservationAdmin(admin.ModelAdmin):
    list_display = ['location', 'source', 'timestamp', 'temperature_celsius', 'quality_flag']
...
class CurrentObservationAdmin(admin.ModelAdmin):
    list_display = ['location', 'source', 'timestamp', 'speed_ms', 'direction_degrees', 'quality_flag']
    list_filter = ['source', 'quality_flag', 'timestamp']
    search_fields = ['location__name']
    readonly_fields = ['created_at']
    date_hierarchy = 'timestamp'
...
class IngestionLogAdmin(admin.ModelAdmin):
    list_display = ['source', 'status', 'started_at', 'completed_at', 'records_inserted']
